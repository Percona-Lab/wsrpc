...
func (c *{{ $.ServiceNameUnexported }}Client) {{ .Name }}(ctx context.Context, req *{{ .InputType }}) (*{{ .OutputType }}, error) {
...
func dispatch{{ .Name }}(server interface{}, arg []byte) ([]byte, error) {
	...
	res, err := server.({{ $.ServiceName }}Server).{{ .Name }}(context.TODO(), req)
	if err != nil {
		return nil, err
	}
...
func (d *{{ .ServiceName }}Dispatcher) Run(ctx context.Context) {
	for {
